...
from Dataset.dataset_classification import read_data,DatasetECG
import argparse
...
parser.add_argument('--direction',type=str,default='FordA')
...
parser.add_argument('--outlen', type=int, default=0)
...
for seed in args.seed:
    ...
    path = 'Data/UCRArchive_2018'+'/'+args.direction
    trainfile = path+'/'+args.direction+'_TRAIN.tsv'
    testfile = path+'/'+args.direction+'_TEST.tsv'
    train_data,val_data,train_label,val_label = read_data(trainfile,val=True,seed=seed,rate=args.rate)
    test_data,test_label = read_data(testfile,val=False)
    ...
    model = STClass(1,args.num_class,[2,2,2],args.dmodel,args.decomp,args.alpha,args.beta,args.part).to(device)
    ...
    if args.test:
        ...
        for j in range(args.checkpoint_num):
            ...
            model = STClass(1,args.num_class,[2,2,2],args.dmodel,args.decomp, 0.5, 0.5,
                            args.part).to(device)
